...
<body>
    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div class="w-full max-w-md bg-white rounded-xl shadow-lg p-8 border border-gray-100">
            ...
            <h1 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Edit Kereta</h1>
...
1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
            ...
            <form action="{{ route('kereta.update', $kereta->id_kereta) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <!-- Nama Kereta -->
                <div>
                    <label for="nama_kereta" class="block text-sm font-medium text-gray-700 mb-1">Nama Kereta</label>
                    <input type="text" id="nama_kereta" name="nama_kereta"
                        value="{{ old('nama_kereta', $kereta->nama_kereta) }}"
                ...
                <div>
                    ...
                    <input type="text" id="no_kereta" name="no_kereta"
                        value="{{ old('no_kereta', $kereta->no_kereta) }}"
                        class="block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition">
                    @error('no_kereta')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
